...
def save_dataset(id_data_dir, ood_data_dir, save_dir, height=None, width=None):
    ...
    # Separate ID data into training, validation, and test sets
    X_train, X_test, y_train, y_test = train_test_split(data, labels, train_size=0.9, random_state=22, shuffle=True)
    X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, train_size=0.8, random_state=22, shuffle=True)
    ...
    X_train = np.float16(X_train / 255)
    X_val   = np.float16(X_val / 255)
    X_test  = np.float16(X_test / 255)
    X_ood   = np.float16(X_ood / 255)

    y_train = np.int16(y_train)
    y_val   = np.int16(y_val)
    y_test  = np.int16(y_test)
    y_ood   = np.int16(y_ood)

    # Save all data to a npz file
    file = os.path.join(save_dir, 'dataset.npz')
    np.savez(file, train_data=X_train, train_labels=y_train,
                    val_data=X_val, val_labels=y_val,
                    test_data=X_test, test_labels=y_test,
                    ood_data=X_ood, ood_labels=y_ood)
